    ...
    for idr2 = 1:repeats % independent runs (initial value)
        ...
        for idx_model = idxes_model_dm
            ...
            if isequal(model_desc.fn, @dppca)
                model_dm = model_desc.fn(mm_trans_t, V, E, M, ETA, THRESHd, m_init4);
            elseif isequal(model_desc.fn, @sdppca)
                model_dm = model_desc.fn(mm_trans_t, V, E, M, ETA, model_desc.type, THRESHd, m_init4, 10000, 'fix_mu');
            else
            ...
            save_parfor_model_init_angle(path_result, model_dm, m_init4, max_angle);    
        end
