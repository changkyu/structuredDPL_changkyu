function [W_new, MU_new, PREC_new, EZn, EZnZnt] = sdppca_m_local(...
    Xi, MissIDXi, M, idx, Bj, ETAi, Wi, MUi, PRECi, LAMBDAi, GAMMAi, BETAi)
% DPPCA_M_LOCAL  Distributed Probablistic PCA (D-PPCA) Local Node with
%                Missing Values
% 
% Description
%  model = sdppca_m_local(.) solves local optimization problem using
...
% [NEW]
    W_new4 = sum(EZnZnt(:,:,NcI), 3) * PRECi(idx) + 2*sumETAi*eye(M);
    for jn = 1:cBj
        W_new(d,:) = (PRECi(idx) * W_new1 - W_new2 + ETAi(Bj(jn)) * W_new3) / W_new4;
    end
end
...
% [NEW]
        MU_new3 = MU_new3 + ETAi(Bj(jn))*( MUi(d,idx) + MUi(d,Bj(jn)) );
    end
% [ORG]
%    MU_new4 = sum(NcI) * PRECi(idx) + 2 * ETA * cBj;
% [NEW]
    MU_new4 = sum(NcI) * PRECi(idx) + 2 * sumETAi;
    
% [ORG]
%    MU_new(d) = (MU_new1 - MU_new2 + ETA * MU_new3) / MU_new4;
% [NEW]
    MU_new(d) = (MU_new1 - MU_new2 + MU_new3) / MU_new4;
end
